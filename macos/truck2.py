...
df = pd.read_excel(r'/Users/roshaanabbasjaffery/Downloads/1212 (1).xlsx', sheet_name= 'Sayfa1', header  = 1)
df = df.iloc[:,:17]
df = df.dropna()
...
if np.sum(w) > w_lim:
    ...
    model.Maximize(sum( x[i] for i in I))  
    ...
    for i in range(len(x_val)):
        xi = np.linspace(x_val[i], x_val[i]+sizes_x[i]//2)
        yi = np.linspace(y_val[i], y_val[i]+sizes_y[i]//2)
        zi = np.linspace(z_val[i], z_val[i]+sizes_z[i]//2)
    ...
    for i, label in enumerate(x_val):
        if z_val[i] >= 125:
            ax.text(y_val[i], x_val[i]+20, int(lbl[i]), fontsize=10, color='red',
                    ha='center', va='center')
        else:
            ax.text(y_val[i], x_val[i]+10, int(lbl[i]), fontsize=10, color='blue',
                    ha='center', va='center')
    ...
    # with open('Results.txt', 'w') as f:
        ...
        for i in range(len(x_val)):
            ...
            if z_val[i]>=125:
                ...
                text = f'{int(lbl[i])}'
                text_p = f"{int(lbl[i])} ({sizes_x[i]}x{sizes_y[i]})"
                text_list_above.append((str(text_p), text_x, text_y, text_z))
            else:
                ...
                text = f'{int(lbl[i])}'
                text_p = f"{int(lbl[i])} ({sizes_x[i]}x{sizes_y[i]})"
                text_list_below.append((str(text_p), text_x, text_y, text_z))
    ...
    text_sequence_below_125 = '------'.join(map(lambda x: str(x[0]), sorted_text_list_below))
    
    print("Aşağı:")
    print()
    print(text_sequence_below_125)
    print()

    text_sequence_above_125 = '------'.join(map(lambda x: str(x[0]), sorted_text_list_above))
    print("Üstte: ")
    print()
    print(text_sequence_above_125)
